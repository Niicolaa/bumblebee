...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `baseline` | Homebrew `Cellar` / `Caskroom` install metadata and lib prefixes; `/Library/Python`; Linux system Python (`/usr/lib/python*`, plus `/usr/local/lib`); user Python (`~/.local/lib/python*`, `~/.local/share/pipx/venvs`, `pyenv`); language version managers (`asdf`, `nvm`, `rbenv`, `rvm`); `~/.cargo` (incl. the registry source cache); `~/go`; `~/.nuget/packages`; `~/.m2/repository`; global npm-family prefixes (`~/.npm-global`, Yarn/pnpm/Bun global roots, `%APPDATA%\npm` on Windows); per-user Python (`~/Library/Python/*/lib/python/site-packages` on macOS, `~/.local/lib/python*`, pipx and `uv tool` dirs); per-user conda; Composer global (`~/.composer/vendor`, `~/.config/composer/vendor`); Dart `~/.pub-cache`; Elixir `~/.hex/packages`; Gradle's module cache (`~/.gradle/caches/modules-2/files-2.1`); toolchains relocated by environment variable (`GOPATH`/`GOMODCACHE`, `CARGO_HOME`, `NPM_CONFIG_PREFIX`, `PNPM_HOME`, `GEM_HOME`, `COMPOSER_HOME`, `PUB_CACHE`); editor-extension trees; MCP config locations; agent-skill lock locations (`~/.agents`, `$XDG_STATE_HOME/skills`); per-profile browser-extension trees (Chromium-family + Firefox-family, including common snap/flatpak paths). No project trees.   |
